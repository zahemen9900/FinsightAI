        ...
        if not self.has_paused and self.pause_steps and state.global_step >= self.pause_steps:
            ...
            with Progress(
                ...
                while elapsed < total_seconds:
                    ...
                    # Every minute, also log a message
                    if int(remaining) % 60 == 0 and int(remaining) > 0 and int(elapsed) > 0:
                        mins_remaining = int(remaining // 60)
                        if mins_remaining > 0:  # Avoid duplicate 0 minute messages
                            self.console.print(f"[yellow]Still paused. {mins_remaining} minute{'s' if mins_remaining != 1 else ''} remaining until training resumes.[/yellow]")
